var mysqlReplicator =  function(opts, readyCallBack) {
    var options = opts;
        ...
        return new Promise(function(resolve, reject) {
            ...
            connection.query(sql, function(err, result) {
                ...
                result.forEach(function(row) {
                    ...
                    } else if(row.Type.toLowerCase().indexOf("json") > -1) {
                        row.Size = 4294967295;
    ...
    /// Let mysql know what database we're workig with
    async function useDatabase(dbName) {
        return new Promise(function(resolve, reject){
            connection.end();
            options.db.database = dbName;
            connection = mysql.createConnection(options.db);
    ...
    async function getInsertOrUpdateSQL(jsonObject) {
        ...
        for(var i = 0; i < table.length; i++) {
            ...
            valueString += mysql.escape(value);
            if(i < (table.length - 1)) {
                valueString += ',';
            }
        }
    ...
    return {
        ...
        storeSequence: async function(sequence) {
            return new Promise(function(resolve, reject) {
                connection.query(sqls.updatesequence.replace("{0}", mysql.escape(sequence)), function(err) {
                    if(err) { reject (err); return; }
                    resolve(true);
                })
